    ...
    class GotoRelevantFile < Request
      ...
      def relevant_filename_pattern(path)
        ...
        relevant_basename_pattern =
          if input_basename.end_with?("test") || input_basename.end_with?("spec")
            input_basename.gsub(/(_test|_spec|_expectations_test|_integration_test)$/, "")
          else
            "#{input_basename}{_test,_spec,_expectations_test,_integration_test}"
          end
