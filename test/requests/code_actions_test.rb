...
class CodeActionsTest < Minitest::Test
  def test_diagnostic_calls_are_cached
    ...
    # If diagnostics is being properly cached, then we should only see `run` invoked once for the
    # same URI, no matter how many times we invoke it and with which range
    RubyLsp::Requests::Support::RuboCopDiagnosticsRunner.instance.expects(:run).once.returns([])
