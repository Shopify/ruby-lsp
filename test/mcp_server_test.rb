...
module RubyLsp
  class MCPServerTest < Minitest::Test
    ...
    def teardown
      ...
      socket_path = File.join(MCPServer::MCP_FOLDER, File.basename(@global_state.workspace_path))
    ...
    def test_handle_get_methods_details_instance_method
      ...
      result_yaml = YAML.safe_load(
        result[0][:text],
        permitted_classes: [Symbol, URI::File, RubyIndexer::Entry::Visibility],
      )
    ...
    def test_handle_get_class_module_details_class
      ...
      result_yaml = YAML.safe_load(result[0][:text], permitted_classes: [Symbol, URI::File])
